...
Выберите раздел для управления:`;
        ...
        const keyboard = [
            ...
            [{ text: '📢 Обязат��льные каналы', callback_data: 'admin_channels' }],
...
Выберите действие:`;
    ...
    async showAllUsers(chatId, userId, page = 1) {
        ...
        try {
            ...
            let message = `👥 Все пользователи (стр. ${page}/${totalPages}):\n\n`;
...
Для разблоки��овки обратитесь к админис��ратору.`);
            } catch (error) {
...
Примеры:
• STARS10 10 100 - код на 10 звёзд, 100 использований
...
${type}`, {
    ...
    async showTaskManagement(chatId, userId, messageId = null) {
        ...
        const message = `📋 У��равление заданиями
...
`;
            ...
            await this.bot.sendMessage(chatId, '❌ Ошибка при загрузке неактивных заданий');
        }
    }
...
Создавайте и управляйте лотереями для пользователей.`;
        ...
        const keyboard = [
            ...
            [
                ...
                { text: '🏆 ��авершить лотер��ю', callback_data: 'admin_lottery_finish' }
            ],
            [
                { text: '🔙 Назад в админку', callback_data: 'admin_panel' }
            ]
...
👤 **${userName}** (ID: ${targetUserId}) был полностью удален из системы.
...
\`🐶 Пёс��к|Больше за рефералов|100|referral_1|2|10\`
...
📊 Макс. уровень: ${maxLevel}`, {
    ...
    async editPetForm(chatId, userId, petId, messageId = null) {
        ...
        try {
            ...
            const message = `✏️ **РЕДАКТИРО��АНИЕ ПИТОМЦА**
...
• Вся связанная статистика`;
    ...
    async deleteAllTasks(chatId, userId, messageId = null) {
        ...
        try {
            ...
            // Удаляем все связанны�� записи о выполнении
            ...
            const successMsg = `✅ ВСЕ ЗАДАНИЯ УДАЛЕНЫ!
...
С кнопками: "🏠 Главное меню" и "👥 Пр��гласить друга"`;
...
⏰ Подведение итогов каждо�� воскресенье в 20:00`;
...
Питомец добавлен в магазин и доступен пользователям!`, {
            ...
            await this.bot.answerCallbackQuery(callbackQuery.id, '❌ Ошибка при создании');
    ...
    async finishLotteryManual(chatId, userId, lotteryId, messageId = null) {
        ...
        try {
            ...
            if (participants.length === 0) {
                await this.bot.sendMessage(chatId, '❌ В лотерее нет у��астников');
                return;
            }
